...
app.use(cors({
  origin: process.env.NODE_ENV === 'production'? 'https://hasnascloset.netlify.app' :'http://localhost:3000',
}));
...
async function run() {
  try {
    ...
    app.post('/api/products/addProducts', upload.single('image'), async (req, res) => {
      ...
      const slug = req.body.slug + shortid()
    ...
    app.put('/api/edit-banner-category/:id', upload.single('banner'), async (req, res) => {
      if (req.file) {
        ...
        const options = { upsert: true };
        const result = await bannerCollection.updateOne(filter, update, options);
        res.send(result)
      } else {
